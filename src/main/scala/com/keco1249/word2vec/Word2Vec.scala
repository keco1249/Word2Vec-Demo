...
// Word2Vec model borrowed from https://github.com/trananh/word2vec-scala
//
...
class Word2Vec {
  ...
  def dist(word1:String, word2:String):(String, String, Double) = {
    ...
    if(!contains(word1) || !contains(word2)){
      println("Out of dictionary words!")
      null
    }
    
    val wordVector1 = vocab.get(word1)
    val wordVector2 = vocab.get(word2)
    
    if(wordVector1 == None || wordVector2 == None){
      println("Error loading vectors")
      null
    }
    
    val dist = cosine(wordVector1.get, wordVector2.get)
